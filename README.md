...
If you are running the app firsttime, you will need to use npm install command, otherwise, just cd to the target document and run npx expo start
```bash
cd moving_wallet
npm install
npx expo start
...
## Option 2 — Use the AWS server (no Docker needed)

We have the whole backend deployed on AWS already, so if you just want to see it working without setting anything up locally, you can use that instead.
